...
contract PolyLend is PolyLendEE, ERC1155TokenReceiver {
    ...
    /// @notice per second rate equal to roughly 2% of end of first day. note that it compounds further. APY ~118K% (118485)
    uint256 public constant MAX_INTEREST = InterestLib.TWO_PERCENT_DAILY_RATE;
    ...
    function accept(uint256 _offerId) external returns (uint256) {
        ...
        loans[loanId] = Loan({
            ...
            minimumDuration: requests[requestId].minimumDuration, // fix: prev using offerId which was wrong
            callTime: 0
        });
        ...
        offers[_offerId].lender = address(0); // fix: prev using requestId which was wrong
